...
def _create_violation_error(
    ...
    if contract.error is None:
        try:
            msg = _represent.generate_message(
            ...
            parts.append(_represent.represent_condition(condition=contract.condition))
...
async def _capture_old_async(
    ...
    for snap in snapshots:
        ...
        # in the meta-class.
        assert snap.name not in old_as_mapping, (
...
def decorate_with_checker(func: CallableT) -> CallableT:
    ...
    assert not hasattr(wrapper, "__preconditions__"), (
    ...
    assert not hasattr(wrapper, "__postconditions__"), (
...
def add_invariant_checks(cls: AnyClass) -> None:
    ...
    assert isinstance(last_invariant, Invariant)
